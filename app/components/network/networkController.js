...
.controller('NetworkController', ['$scope', '$state', '$stateParams', '$filter', 'Config', 'Network', 'Container', 'ContainerHelper', 'Messages', 'Authentication',
function ($scope, $state, $stateParams, $filter, Config, Network, Container, ContainerHelper, Messages, Authentication) {

  $scope.removeNetwork = function removeNetwork(networkId) {
    if(Authentication.getUserDetails().role==2) {
